...
try:
	...
	print "Need numpy to continue. Exiting..."
	exit()
import math
try:
	from PIL import Image,ImageColor
except:
	print "Python PIL library needed ton continue. Exiting..."
	exit()

# Bresenham's line finding algorithm to get the
# pixels in the glow line
...
# a generator to obtain the 4 neighbouring pixels
...
# a generator to obtain all 8 neighbouring pixels
...
# the generic shouldMove fn that calls the required
# shouldMove fn
...
# based on gray scale with threshold 
...
# Generates boundary by identifying all pixels with
# a transparent neighbour. It then checks all the
# neighbours of boundary pixels and if they are transparent
# marks them as boundary pixels as well.
...
# To remove noise, all pixels who have less than 
# 2 neighbours are identified and removed this process
# runs till there is no change in the image
...
# Provides a radial glow to the identified object by
# first finding the center of object and then using
# bresenham's line drawing algorithm to get n pixels 
# for each pixel from the boundary and increasing the
# opacity gradually
...
# Divides the image into 4 equal parts and returns the
# threshold for each of them in a [2x2] matrix
# threshold of each segment is a function of difference
# of maximum intensity color and minimum intensity color
# in the histogram of that part. The histogram has been
# generated by taking a 8px wide and corresponding pixels
# high thumbnail of the image
# 4 thresholds have been generated so that each part gets 
# better edited. More number of parts cannot be created,
# because many background areas will remain
...
# The main function that has to be called
# Returns the edited image and threshold [2x2] matrix
